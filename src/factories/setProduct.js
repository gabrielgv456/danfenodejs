...
export function setProduct(produtos, danfeInput) {
    ...
    produtos.map((itemNF, index) =>
        danfeInput.adicionarItem(new danfe.Item()
            ...
            .comOCst((itemNF.imposto?.[0].ICMS?.[0].ICMS00?.[0].orig?.[0] ?? itemNF.imposto?.[0].ICMS?.[0].ICMS40?.[0].orig?.[0] ?? '' ) + (itemNF.imposto?.[0].ICMS?.[0].ICMS00?.[0].CST?.[0] ?? itemNF.imposto?.[0].ICMS?.[0].ICMS40?.[0].CST?.[0] ?? '') )
