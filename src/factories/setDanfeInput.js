...
export function setDanfeInput(dataNf, emitente, destinatario, transportador, protocolo, impostos, volumes) {
    ...
    const infNF = dataNf.nfeProc.NFe[0].infNFe[0].ide[0]
    const totalNF = dataNf.nfeProc.NFe[0].infNFe[0].total[0].ICMSTot[0]
    ...
    danfeInput.comDataDaEmissao(new Date(infNF?.dhEmi?.[0] ?? '').toISOString().slice(0, -5));
    danfeInput.comDataDaEntradaOuSaida(new Date(infNF?.dhSaiEnt?.[0] ?? '').toISOString().slice(0, -5));
    danfeInput.comModalidadeDoFrete(dataNf.nfeProc.NFe[0].infNFe[0].transp?.[0].transporta?.[0]?.modFrete?.[0] ?? '');
    //danfeInput.comInscricaoEstadualDoSubstitutoTributario('102959579');
    danfeInput.comInformacoesComplementares(dataNf.nfeProc.NFe[0].infNFe[0].infAdic?.[0].infAdFisco?.[0] ?? '')
